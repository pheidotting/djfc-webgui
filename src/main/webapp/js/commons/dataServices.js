        ...
        return {
            ...
            opslaanRelatie: function(data){
                ...
                $.when(this.voerUitGet(navRegister.bepaalUrl('TRACKANDTRACEID'))).then(function(trackAndTraceId){
                    _this.voerUitPost(url, ko.toJSON(data), trackAndTraceId).done(function(response){
                        var id = response.entity.foutmelding;
                        var aantalRequests = 3;

                        if(adressen.length > 0) {
                            $.each(adressen, function(i, adres){
                                adres.entiteitId(id);
                            });
                            _this.voerUitPost(navRegister.bepaalUrl('OPSLAAN_ADRESSEN'), ko.toJSON(adressen), trackAndTraceId).done(function(){
                                retuggevenRelatie();
                            });
                        } else {
                            _this.voerUitPost(navRegister.bepaalUrl('VERWIJDER_ADRESSEN') + '/RELATIE/' + id,null, trackAndTraceId);
                        }
                        if(telefoonnummers.length > 0) {
                            $.each(telefoonnummers, function(i, telefoonnummer){
                                telefoonnummer.entiteitId(id);
                            });
                            _this.voerUitPost(navRegister.bepaalUrl('OPSLAAN_TELEFOONNUMMERS'), ko.toJSON(telefoonnummers), trackAndTraceId).done(function(){
                               retuggevenRelatie();
                           });
                        } else {
                            _this.voerUitPost(navRegister.bepaalUrl('VERWIJDER_TELEFOONNUMMERS') + '/RELATIE/' + id,null, trackAndTraceId);
                        }
                        if(rekeningnummers.length > 0) {
                            $.each(rekeningnummers, function(i, rekeningnummer){
                                rekeningnummer.entiteitId(id);
                            });
                            _this.voerUitPost(navRegister.bepaalUrl('OPSLAAN_REKENINGNUMMERS'), ko.toJSON(rekeningnummers), trackAndTraceId).done(function(){
                               retuggevenRelatie();
                           });
                        } else {
                            _this.voerUitPost(navRegister.bepaalUrl('VERWIJDER_REKENINGNUMMERS') + '/RELATIE/' + id,null, trackAndTraceId);
                        }

                        function retuggevenRelatie(){
                            log.debug('Request nog te gaan ' + --aantalRequests);
                            if (aantalRequests === 0) {
                            log.debug("return");
